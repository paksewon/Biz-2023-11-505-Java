...
/*
 * printStudent() method 를 생성하고
 * 
 * 학생정보를 입력하는 도중 QUIT 를 입력하면 students 리스트에 담긴 학생 정보를 출력
 */
...
public class StudentServiceV3A {
	...
	/*
	 * 학번을 매개변수로 전달받아 students 리스트에서 검색하여 일치하는 학생정보가 있으면 그 정보를 통째(dto)로 return
	 */
	private StudentDto selectStdNum(String num) {
		/*
		 * students 는 List type 이다 students 의 요소는 StudentDto type 이다 이 명령은 students 리스트
		 * 개수만큼 반복하라 반복하면서 students 리스트의 개별 요소를 get(읽기) 하여 StudentDto type인 dto 에 할당하여
		 * 코드블럭({})에 전달하라
		 */
	...
	public boolean inputStudent() { // void 를 boolean 으로 바꾼고 뒤에 false 적고 마지막에 return true
		...
		while (true) {
			stdDto.num = this.itemInput("학번"); // null 값 리턴하고 끝내라
			if (stdDto.num == null)
				return false; // if 한줄만 쓸땐 중괄호 생략가능

			// selsectStdNum() method 에 학번을 보내고
			// return 값을 받아서
