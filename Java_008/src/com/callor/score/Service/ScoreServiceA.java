...
/* 1번문제
 * "src/com/callor/score/Service/data.txt"; 파일을 open
 * data.txt 파일을 한 라인씩 읽어 와서 문자열 변수 line 에 할당하기
 * 읽어온 문자열 컴마(,)분해 하여 문자열 배열 line 로 만들기
 * ScoreDto 클래스를 사용하여 scoreDto 객체를 생성하고,
 * lines 배열의 각 요소를 scoreDto 의 각 변수에 할당
 * scoreDto 객체를 scores 배열에 순서대로 저장
 */
/* 2번문제
 * printScores() method 에 다음의 코드를 작성
 * scores 배열에 저장된 데이터를 다음과 같이 출력
 */
...
public class ScoreServiceA {
	 ...
	 * method
	 * 
	 * 클래스 이름과 같고(첫글자 대문자), retrun type 이 없다 static 도 붙일 수 없다
	 * 
	 * 보통은 비어있는(blank) 코드가 자동으로 생성 되지만 필요에 따라 변수, 객체 등을 만드는 코드를 추가할 수 있다
	 */
	public ScoreServiceA() {
		...
		scan = new Scanner(is);
		scores = new ScoreDto[100]; // 100명 학생 성적 저장하는 코드
	} // end ScoreService()
	 ...
	 * socres 배열에 저장된 데이터를 화면에 성적표로 출력하기
	 */
